...
The competency model is the pedagogical lens; the 7-skill keys are the data layer.`,T=`MET topic rules:
...
- If the student's profile mentions nursing or healthcare, treat it as background context, not as the default topic.`,w=r=>{const{student:e,classEvent:s,classEvidence:c,targetProfile:n,studentGoal:l}=r,t=c||{},a=n||{},i=p=>p?"Yes":"No",g=p=>`${p||0} turn${(p||1)===1?"":"s"}`;return`You are the Diagnostic Analyst for a MET English teaching platform.
...
Profile: ${a.label||a.profileName||"not selected"}
Targets: Overall ${a.overallTarget??"?"} | Speaking ${a.speakingTarget??"?"} | Writing ${a.writingTarget??"?"} | Reading ${a.readingTarget??"?"} | Listening ${a.listeningTarget??"?"}

━━━ EVALUATED SKILLS ━━━
Speaking: ${i(t.evaluatedSpeaking)} (${g(t.speakingEvidenceCount)})
Writing: ${i(t.evaluatedWriting)} (${g(t.writingEvidenceCount)})
Reading: ${i(t.evaluatedReading)} (${g(t.readingEvidenceCount)})
Listening: ${i(t.evaluatedListening)} (${g(t.listeningEvidenceCount)})
Grammar: ${i(t.evaluatedGrammar)} (${g(t.grammarEvidenceCount)})
Vocabulary: ${i(t.evaluatedVocabulary)} (${g(t.vocabularyEvidenceCount)})
Test strategy: ${i(t.evaluatedTestStrategy)} (${g(t.testStrategyEvidenceCount)})

━━━ CLASS EVIDENCE ━━━
Transcript/Answer:
${t.studentTranscript||t.studentAnswer||"not provided"}

Performance Notes:
${t.studentPerformance||"not provided"}

Additional Teacher Notes:
${t.additionalNotes||t.teacherNotes||"none"}
...
}`},E=({topic:r,studentLevel:e,questionCount:s=5})=>`You are a retrieval practice designer for a MET English exam preparation course.
Create a mix of retrieval practice questions for a student at level ${e}.

Topic / learning objective: ${r}
Total questions: ${s}
...
}`,S=({exercises:r,studentLevel:e,objective:s=""})=>{const c=(r||[]).slice(0,12).map((n,l)=>`${l+1}. [${n.type}] ${n.prompt||n.question||n.template||n.sentence||n.errorText||n.title||"(no text)"}`).join(`
...
Student level: ${e}
${s?`Learning objective: ${s}`:""}

Exercises to analyse:
${c||"No exercise content provided."}
...
}`},x=r=>{const{student:e,classEvidence:s,targetProfile:c}=r,n=s||{},l=c||{},t=i=>i?"Yes":"No",a=i=>`${i||0}`;return`Return a compact MET diagnosis as valid JSON only. Use evidence honestly. Do not score skills that were not evaluated.
...
Evaluated skills:
- speaking ${t(n.evaluatedSpeaking)} count ${a(n.speakingEvidenceCount)}
- writing ${t(n.evaluatedWriting)} count ${a(n.writingEvidenceCount)}
- reading ${t(n.evaluatedReading)} count ${a(n.readingEvidenceCount)}
- listening ${t(n.evaluatedListening)} count ${a(n.listeningEvidenceCount)}
- grammar ${t(n.evaluatedGrammar)} count ${a(n.grammarEvidenceCount)}
- vocabulary ${t(n.evaluatedVocabulary)} count ${a(n.vocabularyEvidenceCount)}
- testStrategy ${t(n.evaluatedTestStrategy)} count ${a(n.testStrategyEvidenceCount)}

Evidence:
${n.studentTranscript||n.studentAnswer||"not provided"}

Teacher notes:
${n.studentPerformance||n.additionalNotes||n.teacherNotes||"none"}
...
}`},N=r=>{const{student:e,classEvent:s,classEvidence:c,diagnosis:n}=r,l=c||{};return`You are a warm, encouraging, and highly personalized English Tutor.
...
}`},b=r=>Array.isArray(r)?r:[],u=(...r)=>r.map(b).find(e=>e.length)||[],R=r=>{const{student:e,classEvidence:s,diagnosis:c}=r;return`You are a linguistic pattern analyst.
Your job is to:
...
━━━ EVIDENCE ━━━
Transcript/Notes: ${s.studentTranscript||s.studentAnswer||"no transcript"}

━━━ DIAGNOSIS CONTEXT ━━━
${JSON.stringify(c?.skillDiagnosis||{},null,2)}
...
}`};function I(r,e){return w(e)}const $=r=>{const{student:e,dx:s,ratings:c,reflections:n}=r,l=s?.priorityDiagnosis||s?.sections?.priorityDiagnosis?.content||[],t=s?.classSummary||s?.sections?.classSummary?.content||s?.content?.classSummary||"no summary provided";return`You are a Master Pedagogical Coach for MET English teachers.
...
Diagnostic Summary: ${t}
Key Student Needs:
${l.map(a=>`- [${a.urgency}] ${a.area}: ${a.whatToImprove}`).join(`
`)||"No specific priorities recorded."}

━━━ TEACHER'S SELF-EVALUATION ━━━
Ratings (1-4 scale):
${Object.entries(c).map(([a,i])=>`- ${a}: ${i}`).join(`
`)}

Reflections:
- What worked: ${n.workedWell||"not provided"}
- To improve: ${n.toImprove||"not provided"}
- Student target: ${n.studentTarget||"not provided"}
...
}`},v=`Complete exercise JSON requirements:
...
Do not return placeholder text. Do not omit answer keys.`,y="Topics should reflect real-life situations appropriate for the MET exam: education, work, technology, community, lifestyle, environment, personal decisions, public services, general health and wellbeing. Avoid overly niche or culturally specific topics.",C=({student:r,diagnosis:e})=>{const s=u(e?.priorityDiagnosis,e?.sections?.priorityDiagnosis?.content),c=e?.sections?.skillDiagnosis?.content||{},n={speaking:["speak"],writing:["short"],grammar:["fix","blank","mcq"],vocabulary:["blank","mcq"],reading:["read"],listening:["listen"]},l=["speak","short","listen","mcq","blank","fix"],t=Object.entries(c).filter(([,d])=>d?.evaluated).sort((d,m)=>(d[1]?.score0to80??100)-(m[1]?.score0to80??100)).map(([d,m])=>({skill:d,score:m?.score0to80,weaknesses:b(m?.weaknesses)})),a=new Set,i=[];for(const{skill:d}of t)for(const m of n[d]||[])a.has(m)||(i.push(m),a.add(m));for(const d of l)!a.has(d)&&i.length<7&&(i.push(d),a.add(d));const g=i.slice(0,7),p=t.length?t.slice(0,3).map(({skill:d,score:m,weaknesses:h})=>`${d}${m!=null?` (${m}/80)`:""}: ${h.slice(0,2).join(", ")||"needs work"}`).join(" | "):null;return`You are the Architect of a MET Homework set. 
  Design a Strategic Map for ${r?.name||"the student"}.
  
  Priorities: ${s.slice(0,3).map(d=>`${d.area}: ${d.whatToImprove||""}`).join(" | ")||"MET B1-B2 readiness"}.
  ...
  Return JSON only:
  { "title": string, "objective": string, "taskTypes": ${JSON.stringify(g)} }`},O=({student:r,diagnosis:e,taskBlueprint:s,taskType:c})=>{const n=u(e?.teacherMeaning?.priorityDiagnosis,e?.diagnosticData?.priorityRecommendations,e?.priorityDiagnosis,e?.sections?.priorityDiagnosis?.content),l=u(e?.diagnosticData?.errors,e?.errorBank,e?.sections?.errorBankSuggestions?.content),t=u(e?.diagnosticData?.vocabulary,e?.vocabTargets?.vocabularyTargets,e?.sections?.vocabGrammarTargets?.content?.vocabularyTargets),a=u(e?.vocabTargets?.grammarTargets,e?.sections?.vocabGrammarTargets?.content?.grammarTargets);return`You are a MET Specialist Forge for the ${c} task type.
  Generate one complete, high-fidelity MET exam item for ${r?.name||"the student"}.
  
  Target: ${s.objective}.
  Student: ${r?.currentLevel||"B1"} -> ${r?.targetLevel||"B2"}
  Priorities: ${n.slice(0,3).map(i=>`${i.area}: ${i.whatToImprove}`).join(" | ")||"MET readiness"}
  Errors: ${l.slice(0,4).map(i=>`"${i.error}" -> "${i.correct}"`).join(" | ")||"none"}
  Vocabulary: ${t.slice(0,4).map(i=>i.wordOrPhrase).join(", ")||"general MET vocabulary"}
  Grammar: ${a.slice(0,3).map(i=>`${i.area}: ${i.issue}`).join(" | ")||"B1-B2 grammar control"}
  ...
  Return ONLY one valid JSON object for type "${c}".`},L=({student:r,blueprint:e,tasks:s})=>{const c=(s||[]).map(n=>n?.type||n?.skillGroup||"").filter(Boolean).join(", ");return`You are the MET Homework Synthesizer.
  Review and refine this homework set for ${r?.name||"the student"}.
  Blueprint: ${e.title}
  Exercise types: ${c||"mixed"}
  Tasks: ${JSON.stringify(s)}
  ...
  Return JSON: { "instructions": string, "selfCheck": [string], "teacherNotes": string }`},A=({student:r,diagnosis:e,group:s,count:c=5})=>{const n=u(e?.teacherMeaning?.priorityDiagnosis,e?.diagnosticData?.priorityRecommendations,e?.priorityDiagnosis,e?.sections?.priorityDiagnosis?.content),l=u(e?.errorBank,e?.sections?.errorBankSuggestions?.content),t=u(e?.vocabTargets?.vocabularyTargets,e?.sections?.vocabGrammarTargets?.content?.vocabularyTargets),a=u(e?.vocabTargets?.grammarTargets,e?.sections?.vocabGrammarTargets?.content?.grammarTargets),g=(e?.sections?.skillDiagnosis?.content||{})[s]||{},p=b(g.weaknesses).slice(0,4),m={speaking:"speak, short",writing:"short, fix",grammar:"fix, blank, mcq",vocabulary:"blank, mcq",reading:"read",listening:"listen",mixed:"mcq, blank, short, fix"}[s]||"mcq, blank, short",h=s.charAt(0).toUpperCase()+s.slice(1),f=s==="listening"?'- MANDATORY: Every exercise MUST be type "listen". Include audioText (2–4 sentence spoken script), question, 4 options, correct index, and explanation. No other types allowed.':s==="reading"?'- MANDATORY: Every exercise MUST be type "read". Include passage (150–250 words), questions array with at least 3 items (each with question, 4 options, correct index, explanation). No other types allowed.':`- Use types: ${m}. Mix them for MET variety. Only MET exam types: mcq, blank, short, speak, order, fix.`;return`You are a MET English exam preparation expert. Generate exactly ${c} structured exercises targeting ${h}.

━━━ STUDENT ━━━
${r?.name||"Student"} | ${r?.currentLevel||"B1"} → ${r?.targetLevel||"B2"}

━━━ SKILL GROUP: ${h.toUpperCase()} ━━━
${p.length?p.map(o=>`- ${o}`).join(`
`):`Target: B1→B2 ${h} skills relevant to MET exam`}

━━━ DIAGNOSIS PRIORITIES ━━━
${n.slice(0,2).map(o=>`- [${o.urgency}] ${o.area}: ${o.whatToImprove}`).join(`
`)||"None recorded."}

━━━ ERRORS TO TARGET ━━━
${(l.filter(o=>(o.category||"").toLowerCase().includes(s)).slice(0,4).length?l.filter(o=>(o.category||"").toLowerCase().includes(s)).slice(0,4):l.slice(0,4)).map(o=>`- "${o.error}" → "${o.correct}" (${o.category})`).join(`
`)||"None recorded."}

━━━ VOCABULARY / GRAMMAR TARGETS ━━━
${s==="vocabulary"||s==="mixed"?t.slice(0,4).map(o=>`- ${o.wordOrPhrase}: ${o.meaning||""}`).join(`
`)||"None.":""}
${s==="grammar"||s==="mixed"?a.slice(0,3).map(o=>`- ${o.area}: ${o.issue}`).join(`
`)||"None.":""}

━━━ RULES ━━━
- Generate exactly ${c} exercises. Every exercise is FULLY WRITTEN — student opens and starts immediately.
${f}
- Use general MET topics by default. ${y}
...
Return ONLY valid JSON — an array of ${c} exercise objects:
[
  {
...
Include only the fields relevant to the exercise type. The "content" field always contains the main exercise text.`},D=({student:r,diagnosis:e,level:s,skill:c})=>{const n=u(e?.teacherMeaning?.priorityDiagnosis,e?.diagnosticData?.priorityRecommendations,e?.priorityDiagnosis,e?.sections?.priorityDiagnosis?.content),l=u(e?.diagnosticData?.errors,e?.errorBank,e?.sections?.errorBankSuggestions?.content),t=u(e?.diagnosticData?.vocabulary,e?.vocabTargets?.vocabularyTargets,e?.sections?.vocabGrammarTargets?.content?.vocabularyTargets),a=u(e?.vocabTargets?.grammarTargets,e?.sections?.vocabGrammarTargets?.content?.grammarTargets),i=s?`
Target CEFR level: ${s}`:"",g=c==="speaking",p=c==="writing",d=g?5:p?2:7,f=g?`
...
T2 | metTask:"T2" | targetWords:250 | type:"short"
  ...
  B2 connectors to encourage: In addition, Furthermore, This is because, For instance, However, Overall, In conclusion.`:c?`
Skill focus: ${c}`:"";return`You are a MET English exam preparation expert. Generate a menu of ${d} distinct, ready-to-use exercises for the teacher to choose from.${i}${f}

━━━ STUDENT ━━━
Name: ${r?.name||"Student"}
Level: ${r?.currentLevel||"B1"} → Target: ${r?.targetLevel||"B2"}

━━━ DIAGNOSIS PRIORITIES ━━━
${n.slice(0,3).map(o=>`- [${o.urgency}] ${o.area}: ${o.whatToImprove}`).join(`
`)||"No priorities recorded."}

━━━ ACTIVE ERRORS ━━━
${l.slice(0,6).map(o=>`- "${o.error}" → "${o.correct}" (${o.category})`).join(`
`)||"None recorded."}

━━━ VOCABULARY TARGETS ━━━
${t.slice(0,5).map(o=>`- ${o.wordOrPhrase}`).join(`
`)||"None."}

━━━ GRAMMAR TARGETS ━━━
${a.slice(0,3).map(o=>`- ${o.area}: ${o.issue}`).join(`
`)||"None."}
...
]`},_=({student:r,diagnosis:e,taskBlueprint:s})=>{const c=u(e?.teacherMeaning?.priorityDiagnosis,e?.diagnosticData?.priorityRecommendations,e?.priorityDiagnosis,e?.sections?.priorityDiagnosis?.content),n=u(e?.diagnosticData?.errors,e?.errorBank,e?.sections?.errorBankSuggestions?.content),l=u(e?.diagnosticData?.vocabulary,e?.vocabTargets?.vocabularyTargets,e?.sections?.vocabGrammarTargets?.content?.vocabularyTargets);return`You are a MET English Instructional Designer specializing in Listening comprehension.
...
━━━ DIAGNOSIS PRIORITIES ━━━
${c.slice(0,3).map(t=>`- [${t.urgency}] ${t.area}: ${t.whatToImprove}`).join(`
`)||"none"}

━━━ TARGET ERRORS & VOCAB ━━━
Errors: ${n.slice(0,4).map(t=>`"${t.error}" → "${t.correct}"`).join(", ")||"none"}
Vocab: ${l.slice(0,4).map(t=>t.wordOrPhrase).join(", ")||"general MET vocabulary"}
...
}`},M=({student:r,diagnosis:e,questionCount:s=3})=>{const c=u(e?.priorityDiagnosis,e?.sections?.priorityDiagnosis?.content),n=u(e?.vocabTargets?.vocabularyTargets,e?.sections?.vocabGrammarTargets?.content?.vocabularyTargets),l=r?.currentLevel||"B1",t=r?.targetLevel||"B2";return`You are a MET English Instructional Designer specializing in Reading comprehension.
Build one complete, student-ready reading exercise with a passage and ${s} comprehension questions.
...
━━━ DIAGNOSIS PRIORITIES ━━━
${c.slice(0,3).map(a=>`- [${a.urgency}] ${a.area}: ${a.whatToImprove}`).join(`
`)||"none"}

━━━ TARGET VOCAB ━━━
${n.slice(0,6).map(a=>a.wordOrPhrase).join(", ")||"general MET academic vocabulary"}

━━━ RULES ━━━
1. PASSAGE: Write a realistic 120–200 word passage at ${l}–${t} level.
   - Use an informational or opinion text on a general interest topic.
   - ${y}
   - Weave 2–3 of the target vocab words in naturally.
2. QUESTIONS: Write exactly ${s} MCQ comprehension questions.
...
5. VOCABULARY: Include 3–5 key vocabulary words from the passage at ${l}–${t} level. These should be useful academic or topic-specific words the student should learn.
...
}`};export{x as a,w as b,N as c,R as d,I as e,C as f,O as g,L as h,S as i,D as j,M as k,_ as l,A as m,E as n,$ as o};
