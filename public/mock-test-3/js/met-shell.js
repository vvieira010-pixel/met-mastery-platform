...
(function () {
  ...
  function _initAnswerOptions() {
    document.addEventListener('click', e => {
      ...
      _updateTopBar();
      _updateSidebar();
      // Key answers by the question's stable id (data-group) so parts of a
      // section never collide in the shared met:answers:v1 store.
      _persistAnswer(opt.dataset.group || ('p' + MET.currentPageIdx), opt.textContent.trim());
    });
  }

  function _persistAnswer(questionKey, optionText) {
    try {
      const store = JSON.parse(localStorage.getItem('met:answers:v2') || '{}');
      store[MET.timerStorageKey + '__' + questionKey] = optionText;
      localStorage.setItem('met:answers:v2', JSON.stringify(store));
    } catch (_) {}
  }
    ...
    document.querySelectorAll('textarea[data-word-count-target]').forEach(ta => {
      ...
      ta.addEventListener('input', () => {
        ...
        if (words > 0) {
          ...
          _updateTopBar();
          _updateSidebar();
          // Key by the textarea's stable id so Task 1 / Task 2 (shared
          // met:writing3 storage key) never collide in met:answers:v1.
          _persistAnswer(ta.dataset.wordCountTarget || ('p' + MET.currentPageIdx), ta.value.trim());
        }
      });
    });
  }
