 ...
 *
 * Run one skill at a time (the inexpensive gateway tier is rate-limited):
 *   bun tests/assemblyai-scoring.selftest.mjs
 *   ASSEMBLYAI_SELFTEST_SKILL=speaking bun tests/assemblyai-scoring.selftest.mjs
 * Requires ASSEMBLYAI_API_KEY (sourced from .env).
 */
import { callAssemblyAILLMJson, extractScores } from '../api/_routes/_assemblyai-llm.js';
import { buildExaminerPrompt as writingPrompt, rubricToScaled as writingScaled } from '../api/_routes/_met-writing-scale.js';
import { buildExaminerPrompt as speakingPrompt, rubricToScaled as speakingScaled } from '../api/_routes/_met-speaking-scale.js';
...
const skill = process.env.ASSEMBLYAI_SELFTEST_SKILL || 'writing';

async function run() {
  if (!['writing', 'speaking'].includes(skill)) {
    console.error('ASSEMBLYAI_SELFTEST_SKILL must be "writing" or "speaking".');
    process.exitCode = 1;
    return;
  }
  ...
  if (skill === 'writing') {
    ...
    const scores = extractScores(w.evaluation, W_KEYS);
    const conv = writingScaled(W_KEYS.reduce((sum, key) => sum + Math.min(4, Math.max(0, scores[key])), 0) / W_KEYS.length);
  ...
  console.log('── SPEAKING (AssemblyAI LLM Gateway) ──');
