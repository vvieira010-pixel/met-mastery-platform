...
const NEW_NAV_SECTIONS = [
  ...
  { label: 'Admin',    ids: ['operations', 'settings'] },
];
  ...
  return (
    <div className="shell">
      ...
      <header className="shell-topbar">
        <button type="button" className="shell-brand" onClick={() => onTab('dashboard')} aria-label="MET Mastery teacher home">
          <span className="shell-brand-mark" aria-hidden="true">M</span>
          <span className="shell-brand-copy"><strong>MET Mastery</strong><small>Teacher workspace</small></span>
        </button>
        ...
        {/* Persistent teaching cycle indicator — always visible */}
        <WorkflowStageStrip active={workflowActive || 'diagnose'} onStage={onWorkflowStage} persistent showLabels />
            ...
            <div className="mobile-nav-dropdown" style={{ position: 'absolute', bottom: '100%', left: 0, right: 0, background: 'var(--surface)', border: '1px solid var(--border)', borderBottom: 'none', borderRadius: 'var(--radius-lg) var(--radius-lg) 0 0', boxShadow: '0 -4px 20px rgba(0,0,0,0.15)', zIndex: 100, padding: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
              {tabs.filter(t => t.mobile !== false && !['dashboard','students','diagnostics','homework','submissions','library','mock-test-results'].includes(t.id) || ['students','homework','library','mock-test-results'].includes(t.id)).map(tab => (
                <button key={tab.id}
                  className={`shell-mobile-nav-btn${active === tab.id ? ' active' : ''}`}
                  aria-current={active === tab.id ? 'page' : undefined}
                  onClick={() => { onTab(tab.id); setShowMobileOverflow(false); }}
                  style={{ padding: '12px 16px', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 12 }}>
                  {tab.icon}
                  <span>{tab.label}</span>
                </button>
              ))}
