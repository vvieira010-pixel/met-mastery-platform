/**
 * exercise-editor.jsx — Teacher-facing exercise editors for all types.
...
import { EX_TYPES, LISTENING_FORMATS, getExType } from '../lib/exercise-types.js';
  ...
  return (
    <div>
      ...
      <div style={fieldWrap}>
        ...
        {ex.imageUrl && (
          <div>
            <img src={ex.imageUrl} alt={ex.imageAlt || 'preview'} loading="lazy" style={{ width: '100%', height: 'auto', aspectRatio: '16 / 9', borderRadius: 'var(--radius-sm)', marginTop: 4 }} />
...
function SpeakEditor({ ex, update }) {
  ...
  return (<>
      ...
      <div style={fieldWrap}>
        ...
        />
        {ex.imageUrl && (
          <img src={ex.imageUrl} alt={ex.imageAlt || 'preview'} loading="lazy" style={{ marginTop: 8, width: '100%', height: 'auto', aspectRatio: '16 / 9', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)' }} />
        )}
      </div>
      {ex.imageUrl && (
        <div style={fieldWrap}>
...
function ListenEditor({ ex, update }) {
  ...
  return (
    <div>
      ...
      <div style={fieldWrap}>
        <label style={fieldLabel}>Listening activity format</label>
        <select className="input" value={ex.listeningFormat || 'multiple_choice'} onChange={e => update({ listeningFormat: e.target.value })}>
          {LISTENING_FORMATS.map(format => <option key={format.id} value={format.id}>{format.label}</option>)}
        </select>
        <div style={hintText}>{LISTENING_FORMATS.find(format => format.id === (ex.listeningFormat || 'multiple_choice'))?.hint}</div>
      </div>
      <div style={fieldWrap}>
      ...
      {ex.listeningFormat === 'gap_fill' && (
        <div style={fieldWrap}>
          <label style={fieldLabel}>Gap-fill answers</label>
          {(ex.gaps || []).map((gap, i) => (
            <div key={gap.id || i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 6 }}>
              <input className="input" value={gap.prompt || ''} onChange={e => { const gaps = [...(ex.gaps || [])]; gaps[i] = { ...gap, prompt: e.target.value }; update({ gaps }); }} placeholder={`Blank ${i + 1} prompt`} />
              <input className="input" value={(gap.acceptableAnswers || []).join(', ')} onChange={e => { const gaps = [...(ex.gaps || [])]; gaps[i] = { ...gap, acceptableAnswers: e.target.value.split(',').map(v => v.trim()).filter(Boolean) }; update({ gaps }); }} placeholder="Accepted answers, comma-separated" />
            </div>
          ))}
          <Button variant="ghost" size="sm" onClick={() => update({ gaps: [...(ex.gaps || []), { id: `gap_${Date.now()}`, prompt: '', acceptableAnswers: [''] }] })}><Icon.plus size={12} /> Add blank</Button>
        </div>
      )}

      {ex.listeningFormat === 'ordering' && (
        <div style={fieldWrap}>
          <label style={fieldLabel}>Events in correct order</label>
          {(ex.sequenceItems || []).map((item, i) => <input key={i} className="input" value={item} onChange={e => { const sequenceItems = [...(ex.sequenceItems || [])]; sequenceItems[i] = e.target.value; update({ sequenceItems, correctOrder: sequenceItems.map((_, index) => index) }); }} placeholder={`Event ${i + 1} in the correct order`} style={{ marginBottom: 6 }} />)}
          <Button variant="ghost" size="sm" onClick={() => update({ sequenceItems: [...(ex.sequenceItems || []), ''], correctOrder: (ex.sequenceItems || []).map((_, i) => i).concat((ex.sequenceItems || []).length) })}><Icon.plus size={12} /> Add event</Button>
        </div>
      )}

      {ex.listeningFormat === 'paraphrase' && (
        <div style={fieldWrap}>
          <label style={fieldLabel}>Source sentence to paraphrase</label>
          <textarea className="input" rows={2} value={ex.sourceSentence || ''} onChange={e => update({ sourceSentence: e.target.value })} placeholder="The proposal was rejected." />
        </div>
      )}

      <label style={fieldLabel}>Answer options (click radio to mark correct)</label>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 8 }}>
        {(ex.options || ['', '', '', '']).map((opt, i) => (
          <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <input
