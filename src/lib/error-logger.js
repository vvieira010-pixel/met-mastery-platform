import { getSupabaseConfig, readStoredSupabaseSession, parseJwtClaims } from './supabase-storage.js';
...
function buildEntry(error, info = {}) {
  const claims = _session?.access_token ? (parseJwtClaims(_session.access_token) || {}) : {};
  return {
    ...
    role: (info.role || (typeof localStorage !== 'undefined' ? localStorage.getItem('vv:db_role') : '') || ''),
    metadata: {
...
function isIgnorableError(error) {
  ...
  if (/AbortError|aborted/i.test(msg)) return true;
  if (/failed to connect to websocket|vite.*websocket/i.test(msg)) return true;
...
export function logError(error, info = {}) {
  ...
  console.warn('[error-logger]', entry.message, info.component || '');
  logToStorage(entry);
  logToSupabase(entry);
}
