...
export default function ToolPerspectiveDesigner({ student, students, onSelectStudent, onNavigate }) {
  ...
  function handleCopy() {
    ...
    if (!text) return;
    navigator.clipboard?.writeText(text).then(() => window.toast?.('Copied to clipboard.', 'ok')).catch(err => { console.error('Clipboard copy failed:', err); window.toast?.('Failed to copy to clipboard.', 'warn'); });
  }
...
const headline = { fontFamily: 'var(--font-sans)', fontSize: 'var(--text-2xl)', fontWeight: 700, color: 'var(--primary)', margin: '0 0 4px' };
