...
export default function LandingComplete({ onMemberSignIn, onDemoAccess, "data-testid": testId }) {
  ...
  return <div className="landing-preview" data-testid={testId}>
    ...
    <header className="site-header">
      ...
      <nav id="site-nav" className={menuOpen ? 'open' : ''}><a href="#method" onClick={() => setMenuOpen(false)}>The learning loop</a><a href="#how" onClick={() => setMenuOpen(false)}>How it works</a><a href="#teachers" onClick={() => setMenuOpen(false)}>For teachers</a><button type="button" className="mobile-signin" onClick={() => { setMenuOpen(false); onMemberSignIn(); }}>Sign in</button></nav>
      <button className="signin-link" type="button" onClick={onMemberSignIn}>Sign in</button><button className="demo-link" type="button" onClick={() => (onDemoAccess || onMemberSignIn)({ mockDirect: true, role: audience === 'teacher' ? 'teacher' : 'student' })}>Try the demo</button><a className="header-cta" href="#start">Find my starting point <span aria-hidden="true">→</span></a>
    </header>
    <main id="main">
    ...
    <section className="audience-picker" aria-label="Choose your pathway"><p className="eyebrow">CHOOSE YOUR NEXT STEP</p><div><button className={audience === 'student' ? 'selected' : ''} type="button" onClick={() => { setAudience('student'); goToStart(); }}><span>I’m preparing for the MET</span><small>Find my level and build a focused practice plan.</small><b>Start as a student →</b></button><button className={audience === 'teacher' ? 'selected' : ''} type="button" onClick={() => { setAudience('teacher'); goToStart(); }}><span>I support MET learners</span><small>Give students a clearer path and useful review context.</small><b>Explore teacher tools →</b></button></div>
    </section>
    ...
    <section className="product-proof" aria-labelledby="proof-title">
      <div className="proof-intro">
        ...
        <h2 id="proof-title">A closer look <span>at your study plan.</span></h2>
