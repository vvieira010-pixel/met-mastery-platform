...
const DIAGNOSTIC_WHATSAPP_URL = 'https://wa.me/5511997801708?text=Hi%20Vin%C3%ADcius%2C%20I%20want%20to%20book%20a%20MET%20diagnostic.';
...
export default function LandingComplete({ onMemberSignIn, "data-testid": testId }) {
  ...
  return <div className="landing-preview" data-testid={testId}>
    ...
    <section className="hero" id="top">
      <div className="hero-copy"><div className="hero-proof-pills"><span>MET test preparation</span></div><h1>Prepare for the MET with a plan that shows you what to work on next.<br /><span>Reach your target score. Speak and write with more confidence in healthcare settings.</span></h1><p>Get individual support for MET Speaking and Writing. We identify the gaps affecting your score, practise the skills that need attention, and use clear feedback to improve each attempt.</p><div className="hero-actions"><a className="hero-primary" href="#start">Book an RN diagnostic <span>→</span></a><a className="hero-secondary" href="#how">See how it works <span>↓</span></a></div><ul><li>Practise Speaking and Writing with MET-style questions</li><li>Build speed with timed exercises</li><li>Understand your grammar, vocabulary, pronunciation, and structure mistakes</li><li>Follow a study plan based on your current performance</li></ul><strong className="hero-question">Not sure why your MET score is not improving? Let’s find the gap first.</strong></div>
      <div className="hero-dashboard" role="img" aria-label="Illustrative diagnostic dashboard preview"><p>YOUR CURRENT PLAN <span>In progress</span></p><h2>Week 2 · Keep building</h2><div className="hero-level"><strong>B2</strong><div><b>Current CEFR level</b><small>Based on initial diagnostic · May 15, 2025</small></div><em className="hero-score">Predicted: 81/100</em></div><div className="hero-skill-list">{skills.slice(0, 4).map(([name, score]) => <div className="hero-skill" key={name}><span>{name}</span><i><b style={{width: score}} /></i><em>{score}</em></div>)}</div><small className="hero-next">Next up: <strong>Writing: distinguishing main ideas</strong> <span>(Est. 20 min)</span></small></div>
    </section>
    ...
    <section className="product-proof" aria-labelledby="proof-title">
      <div className="proof-intro">
        <p className="eyebrow">AFTER YOUR DIAGNOSTIC</p>
        <h2 id="proof-title">See how your <span>study plan is built.</span></h2>
        <p className="lead">Your diagnostic shows which skill and question type need attention. The platform then gives you a task to practise, with teacher review when you need it.</p>
      ...
      <div className="workspace-frame">
        <aside className="workspace-nav">
          ...
          <nav aria-label="Student workspace"><a className="active" href="#plan-pane">My plan</a><a href="#practice">Practice</a><a href="#review">Review</a><a href="#plan-pane">Progress</a></nav>
          <div className="nav-help">Need help?<br /><span>Message your teacher</span></div><button type="button" className="profile-button">Sam Carter <span>Student</span><b>⌄</b></button>
        </aside>
        <div className="workspace-main">
          <section className="plan-pane" id="plan-pane">
            ...
            <article className="diagnosis-card"><div className="level-block"><span>Your MET diagnosis</span><strong>B2</strong><p>Good work. You’re building the skills you need for the next level.</p><a href="#plan-pane">View full results →</a></div><div className="skill-list">{skills.map(([name, score]) => <div className="skill-row" key={name}><span>{name}</span><i><b style={{ width: score }} /></i><em>{score}</em></div>)}</div></article>
        ...
        <div className="callout callout-task"><b>Practise the skill that needs attention</b><span>Your queue starts with the skill and question type that needs the most work.</span></div><div className="callout callout-timer"><b>Timed MET practice</b><span>Timed MET-style tasks help you make clear decisions when the clock is running.</span></div><div className="callout callout-feedback"><b>Teacher-reviewed feedback</b><span>Your teacher can point out the problem and give you one correction to practise.</span></div>
      </div>
    </section>
    <section className="method-section" id="method"><div className="method-intro"><p className="eyebrow">HOW THE PLAN WORKS</p><h2>Practise the skill that needs attention.</h2><p>Your diagnostic shows where to begin. You then practise that skill, review the result, and bring speaking or writing work to your teacher when you need another set of eyes.</p></div><div className="method-grid">{methodSteps.map(([number, title, copy]) => <article key={number}><small>{number} / 06</small><h3>{title}</h3><p>{copy}</p></article>)}</div></section>
    <section className="learning-loop" id="how"><p className="eyebrow">HOW YOUR PREPARATION WORKS</p><h2>Start with your current level, then use the feedback.</h2><p className="loop-lead">The workspace connects your diagnostic, practice tasks, and teacher review so you can see what to work on next.</p><div className="loop-steps"><article><small>01 — START WITH EVIDENCE</small><h3>Take a diagnostic.</h3><p>Get a useful, provisional picture of your six MET skills and the question types behind it.</p></article><article><small>02 — PRACTISE THE GAP</small><h3>Work on the weak point.</h3><p>Your practice queue starts with the skill that needs attention, with timed and untimed options.</p></article><article><small>03 — USE THE FEEDBACK</small><h3>Review your next attempt.</h3><p>Use teacher feedback and saved review points to make the next response clearer and stronger.</p></article></div></section>
    <section className="educator-section" aria-labelledby="educator-title"><div className="educator-profile"><img src="/images/vinicius-vieira.png" alt="Profile photo of MET tutor Vinicius Vieira" /><strong>Vinicius Vieira</strong><span>MET Tutor</span><div className="educator-credentials"><span>C2 CEFR Certified</span><span>Cambridge TKT</span></div></div><div><p className="eyebrow">ABOUT THE TUTOR <span aria-hidden="true">•</span> <small>MET Tutor for Healthcare Professionals</small></p><h2 id="educator-title">MET Tutor for Healthcare Professionals.</h2><p>I help healthcare professionals prepare for the English-language demands of the MET and professional communication. We practise patient explanations, clinical scenarios, concise handovers, and the speaking and writing skills needed for the test.</p><div className="educator-points"><span>✓ Professional pathway context</span><span>✓ Healthcare communication practice</span><span>✓ One-on-one feedback and clear next steps</span></div></div></section>
    <section className="final-cta" id="start"><p className="eyebrow">YOUR NEXT STEP</p><h2>Start with a clearer picture.</h2><p>Book a MET diagnostic to discuss your current level, test date, and the most useful place to begin.</p><a href={DIAGNOSTIC_WHATSAPP_URL} target="_blank" rel="noopener noreferrer" aria-label="Book my MET diagnostic via WhatsApp">Book my MET diagnostic <span>→</span></a><small className="cta-reassurance">A practical conversation first. No score promises.</small></section>
    </main>
