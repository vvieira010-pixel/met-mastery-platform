  ...
  if (!started) {
    return (
      <div style={{ textAlign: 'center', maxWidth: 560, margin: '0 auto' }}>
        <p style={{ marginTop: 18, color: 'var(--ink)', fontSize: '1rem' }}>1 reading, 1 listening, 1 writing and 10 grammar questions that adapt to your level — about 10 minutes.</p>
  ...
  if (stage === 'done') {
    ...
    return (
      <div role="status" aria-live="polite" style={{ textAlign: 'center', maxWidth: 560, margin: '0 auto' }}>
        ...
        <p style={{ margin: '14px 0 4px', color: 'var(--ink)', fontWeight: 700 }}>Rough estimate: {band} · Weakest: {weak}</p>
        <p style={{ margin: '0 0 6px', color: 'var(--ink)', fontSize: '0.75rem' }}>Reading {rOk ? '✓' : '✗'} · Listening {lOk ? '✓' : '✗'} · Writing {words.length} words in {sentences.length} sentences</p>
        <p style={{ margin: '0 0 20px', color: 'var(--ink)', fontSize: '0.75rem' }}>A quick estimate, not a MET score — writing content and speaking need a full diagnostic.</p>
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto' }}>
      <div role="progressbar" aria-valuenow={answeredCount + 1} aria-valuemin={1} aria-valuemax={TOTAL} aria-label={`Question ${answeredCount + 1} of ${TOTAL}`} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        <span style={{ font: "0.75rem 'Space Mono',monospace", color: 'var(--ink)' }}>Q{answeredCount + 1}/{TOTAL}{stage === 'grammar' ? ` · Grammar · level ${gCurrent.level}` : ` · ${stage[0].toUpperCase() + stage.slice(1)}`}</span>
        <div className="lp-bar" aria-hidden="true" style={{ flex: 1 }}><i style={{ width: `${((answeredCount + 1) / TOTAL) * 100}%` }} /></div>
      </div>

      {stage === 'reading' && (
        <div>
          <blockquote style={{ margin: '0 0 16px', padding: 16, background: 'var(--on-dark)', border: '1px solid #E8E5DF', borderRadius: 8, fontSize: '0.875rem', color: 'var(--ink)', lineHeight: 1.6 }}>{SC_NOTICE}</blockquote>
      ...
      {stage === 'listening' && (
        <div>
          ...
          <details style={{ marginTop: 12 }}>
            <summary style={{ cursor: 'pointer', color: 'var(--primary)', fontSize: '0.875rem' }}>Read the transcript instead</summary>
            <blockquote style={{ margin: '8px 0 0', padding: 16, background: 'var(--on-dark)', border: '1px solid #E8E5DF', borderRadius: 8, fontSize: '0.875rem', color: 'var(--ink)', lineHeight: 1.6 }}>{SC_TALK}</blockquote>
          </details>
      ...
      {stage === 'writing' && (
        <div>
          <p style={{ fontWeight: 700, color: 'var(--ink)', marginBottom: 10 }}>{SC_WRITING_PROMPT}</p>
          ...
          <p style={{ color: 'var(--ink)', fontSize: '0.75rem' }} aria-live="polite">{words.length} words · {sentences.length} sentences{words.length > 0 && words.length < 25 ? ' — add one more sentence' : ''}</p>
        ...
        return (
          <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 16 }}>
              ...
              <span style={{ font: "0.75rem 'Space Mono',monospace", color: 'var(--ink)', alignSelf: 'center' }}>{gAsked.length}/{G_TOTAL} grammar</span>
            </div>
          </div>
        );
      })()}
  ...
  return (
    <form onSubmit={e => e.preventDefault()}>
      <fieldset style={{ border: 0, margin: 0, padding: 0 }}>
        <legend style={{ fontWeight: 700, color: 'var(--ink)', marginBottom: 10 }}>{legend}</legend>
        <div role="radiogroup" aria-label={legend} style={{ display: 'grid', gap: 8 }}>
          {options.map((opt, i) => (
            <label key={opt} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', border: value === i ? '2px solid #2D7A8C' : '1px solid #E8E5DF', borderRadius: 8, background: 'var(--on-dark)', cursor: 'pointer', fontSize: '0.875rem', minHeight: 44 }}>
              <input type="radio" name={qid} checked={value === i} onChange={() => onPick(i)} />
              <b aria-hidden="true" style={{ color: 'var(--primary)' }}>{String.fromCharCode(65 + i)}</b> {opt}
            </label>
          ))}
        </div>
      </fieldset>
...
export default function LandingPrototype({ onMemberSignIn, 'data-testid': testId }) {
  ...
  return (
    <div data-testid={testId} style={{ background: 'var(--border)', color: 'var(--ink)', font: '1rem/1.6 DM Sans, sans-serif' }}>
      <style>{`
        .lp-skip{position:absolute;left:-9999px;top:auto;width:1px;height:1px;overflow:hidden;background: var(--primary);color: var(--on-dark);padding:10px 16px;border-radius:8px;z-index:9999;font-size:0.875rem;font-weight:700}
        ...
        .lp-eyebrow{color: var(--primary);font-size:0.75rem;font-weight:700;letter-spacing:.14em;text-transform:uppercase}
        ...
        .lp-mark{width:34px;height:34px;display:grid;place-items:center;color: var(--on-dark);background: var(--primary);border-radius:8px 8px 8px 4px;font:700 1rem 'Space Mono',monospace}
        .lp-links{display:flex;gap:28px;color: var(--ink);font-size:0.875rem}.lp-links a:hover{color: var(--primary)}
        ...
        .lp-primary{color: var(--on-dark);background: var(--primary);box-shadow:0 1px 4px rgba(26, 46, 53,.04)}.lp-primary:hover{background:#1F5A67}
        .lp-light{color: var(--ink);background: var(--surface);box-shadow:0 12px 36px rgba(26, 46, 53,.12)}
        .lp-quiet{color: var(--on-dark);padding-inline:0;background:transparent;border:none}
        .lp-hero{color: var(--on-dark);background: var(--ink);position:relative;overflow:hidden}
        ...
        .lp-h1{margin:0;max-width:650px;font:600 clamp(1.85rem,4vw,3rem)/1.05 'Cormorant Garamond',serif;letter-spacing:-.045em;color: var(--on-dark)}.lp-h1 em{color: var(--warning);font-style:normal}
        .lp-lede{max-width:510px;margin-top:27px;color:rgba(255,255,255,.72);font-size:1.125rem;line-height:1.65}
        .lp-proof{display:flex;flex-wrap:wrap;gap:18px 25px;margin-top:31px;color:rgba(255,255,255,.72);font-size:0.75rem}.lp-proof span::before{content:'✓';color: var(--warning);margin-right:7px;font-weight:700}
        .lp-card{background: var(--surface);border:8px solid rgba(255,255,255,.1);border-radius:16px;box-shadow:0 12px 36px rgba(26, 46, 53,.12);transform:rotate(1deg)}
        .lp-cardInner{padding:25px;border-radius:12px;color: var(--ink);background: var(--surface)}
        ...
        .lp-dashLabel{color: var(--ink);font:0.75rem 'Space Mono',monospace;letter-spacing:.03em}
        .lp-status{padding:6px 10px;color:#804E1E;background:#FBF0E4;border-radius:99px;font-size:0.75rem;font-weight:700}
        .lp-score{color: var(--primary);font:700 2.5rem/.9 'Space Mono',monospace;letter-spacing:-.06em}
      ...
      <main id="lp-main">
        <section className="lp-hero" id="top" aria-labelledby="lp-hero-h"><div className="lp-orb" aria-hidden="true" /><div className="lp-wrap lp-hero-grid"><div>
          ...
          <div style={{ display: 'flex', alignItems: 'end', gap: 18, padding: '25px 0 21px' }}><div className="lp-score" aria-label="Provisional level B2">B2</div><div style={{ color: 'var(--ink)', fontSize: '0.75rem', lineHeight: 1.45 }} id="lp-snap-d"><strong style={{ display: 'block', color: 'var(--ink)', fontSize: '0.875rem' }}>Provisional level</strong>Based on your latest<br />diagnostic · 12 May</div></div>
          <div role="list" aria-label="Skill breakdown" style={{ display: 'grid', gap: 10 }}>
            {skills.map(([name, val]) => (
              <div key={name} role="progressbar" aria-valuenow={val} aria-valuemin={0} aria-valuemax={100} aria-label={`${name} ${val} percent`} style={{ display: 'grid', gridTemplateColumns: '94px 1fr 45px', alignItems: 'center', gap: 14, padding: '12px 13px', border: '1px solid #E8E5DF', borderRadius: 8, fontSize: '0.875rem' }}>
                <b>{name}</b><div className="lp-bar" aria-hidden="true"><i style={{ width: `${val}%`, background: name === 'Listening' ? 'var(--warning)' : 'var(--primary)' }} /></div><em aria-hidden="true" style={{ color: 'var(--ink)', fontStyle: 'normal', textAlign: 'right', font: '0.75rem Space Mono, monospace' }}>{val}%</em>
              </div>
            ))}
          </div>
          <p style={{ marginTop: 18, color: 'var(--ink)', fontSize: '0.75rem', textAlign: 'center' }}>Next up: Listening · distinguishing main ideas</p>
        </div></div></div></section>

        <section id="self-check" aria-labelledby="sc-h" style={{ padding: '72px 0', background: 'var(--bg)', borderBottom: '1px solid #E8E5DF' }}><div className="lp-wrap"><p className="lp-eyebrow" style={{ textAlign: 'center' }}>Try before you book</p><h2 id="sc-h" style={{ margin: '0 auto', textAlign: 'center', maxWidth: 600, font: "600 clamp(1.85rem,4vw,3rem)/1.05 'Cormorant Garamond',serif" }}>Thirteen questions. Ten minutes. A rough level.</h2><div style={{ marginTop: 28 }}><SelfCheck /></div></div></section>

        <div style={{ background: 'var(--primary-light)', borderBottom: '1px solid #E8E5DF' }}><div className="lp-wrap" style={{ display: 'flex', alignItems: 'center', gap: 25, padding: '24px 0', flexWrap: 'wrap' }}><span className="lp-eyebrow">One workspace</span><p style={{ color: 'var(--ink)', fontSize: '0.875rem' }}>Built around the skills the MET actually measures.</p><div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }} role="list" aria-label="MET skills">{['Reading','Listening','Writing','Speaking','Grammar','Vocabulary'].map(s => <span key={s} role="listitem" style={{ padding: '5px 10px', color: 'var(--primary)', border: '1px solid #E8E5DF', borderRadius: 99, fontSize: '0.75rem' }}>{s}</span>)}</div></div></div>

        <section id="features" aria-labelledby="lp-feat-h" style={{ padding: '105px 0' }}><div className="lp-wrap" style={{ display: 'grid', gridTemplateColumns: '.8fr 1.2fr', gap: 90 }}><div style={{ maxWidth: 600 }}><p className="lp-eyebrow">Less noise. Better practice.</p><h2 id="lp-feat-h" style={{ margin: 0, font: "600 clamp(1.85rem,4vw,3rem)/1.05 'Cormorant Garamond',serif", letterSpacing: '-.03em' }}>A plan that changes as you learn.</h2><p style={{ marginTop: 22, color: 'var(--ink)', fontSize: '1rem' }}>Good preparation is not a pile of random questions. It is a loop: find the gap, work on it, get useful feedback, and try again.</p></div><div role="list" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, background: 'var(--border)', border: '1px solid #E8E5DF' }}>{features.map(([number, title, text]) => <article key={number} role="listitem" style={{ minHeight: 190, padding: 27, background: 'var(--border)' }}><span aria-hidden="true" style={{ color: 'var(--warning-text)', font: "0.875rem 'Space Mono',monospace" }}>{number}</span><h3 style={{ margin: '23px 0 10px', font: "600 1.5rem 'Cormorant Garamond',serif" }}>{title}</h3><p style={{ color: 'var(--ink)', fontSize: '0.875rem', lineHeight: 1.55 }}>{text}</p></article>)}</div></div></section>

        <section id="methodology" aria-labelledby="lp-method-h" style={{ background: 'var(--ink)', color: 'var(--on-dark)', padding: '105px 0' }}><div className="lp-wrap"><div style={{ maxWidth: 600 }}><p className="lp-eyebrow" style={{ color: 'var(--primary-light)' }}>The learning loop</p><h2 id="lp-method-h" style={{ margin: 0, font: "600 clamp(1.85rem,4vw,3rem)/1.05 'Cormorant Garamond',serif" }}>From “I’m not sure” to “I know what to do.”</h2><p style={{ marginTop: 22, color: 'rgba(255,255,255,.72)' }}>Every part of the workspace points back to a practical next step, so progress feels concrete, not abstract.</p></div><div role="list" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', marginTop: 60, borderTop: '1px solid rgba(226, 240, 243,.18)' }}>{[
        ...
        ].map(([no, t, d]) => <article key={no} role="listitem" style={{ padding: '28px 30px 10px 0', borderRight: '1px solid rgba(226, 240, 243,.18)', marginRight: 30 }}><span style={{ color: 'var(--warning)', font: "0.875rem 'Space Mono',monospace" }}>{no}</span><h3 style={{ margin: '26px 0 12px', color: 'var(--on-dark)', font: "600 1.5rem 'Cormorant Garamond',serif" }}>{t}</h3><p style={{ color: 'rgba(255,255,255,.72)', fontSize: '0.875rem' }}>{d}</p></article>)}</div></div></section>

        <section id="success" aria-labelledby="lp-success-h" style={{ background: 'var(--success-bg)', padding: '105px 0' }}><div className="lp-wrap" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }}><div><p className="lp-eyebrow">Made for real study weeks</p><h2 id="lp-success-h" style={{ margin: 0, font: "600 clamp(1.85rem,4vw,3rem)/1.05 'Cormorant Garamond',serif" }}>Small, visible steps beat last-minute panic.</h2></div><div><blockquote style={{ margin: 0, color: 'var(--ink)', font: "600 clamp(1.85rem,4vw,3rem)/1.08 'Cormorant Garamond',serif", letterSpacing: '-.02em' }}><span aria-hidden="true" style={{ display: 'block', color: 'var(--warning-text)', fontSize: '2.5rem', lineHeight: .5, marginBottom: 8 }}>“</span><p style={{ margin: 0 }}>I finally knew what to practise between lessons, and why it mattered.</p></blockquote><p style={{ marginTop: 25, color: 'var(--ink)', fontSize: '0.875rem' }}>Student using MET Mastery · illustrative experience</p></div></div></section>

        <section id="start" aria-labelledby="lp-cta-h" style={{ padding: '90px 0', textAlign: 'center', background: 'var(--warning-bg)' }}><div className="lp-wrap"><p className="lp-eyebrow">Your next step</p><h2 id="lp-cta-h" style={{ margin: '0 auto', maxWidth: 600, font: "600 clamp(1.85rem,4vw,3rem)/1.05 'Cormorant Garamond',serif" }}>Start with a clearer picture.</h2><p style={{ maxWidth: 500, margin: '20px auto 28px', color: 'var(--ink)' }}>Take a diagnostic, see your current level, and leave with a practical place to begin.</p><button className="lp-btn lp-primary" type="button" onClick={onMemberSignIn}>Sign in to your workspace <span aria-hidden="true">→</span></button></div></section>

        <section id="pricing" aria-labelledby="lp-pricing-h" style={{ padding: '72px 0', background: 'var(--border)', borderTop: '1px solid #E8E5DF' }}><div className="lp-wrap" style={{ maxWidth: 720, textAlign: 'center' }}><p className="lp-eyebrow">Plans</p><h2 id="lp-pricing-h" style={{ margin: 0, font: "600 clamp(1.85rem,4vw,3rem)/1.05 'Cormorant Garamond',serif" }}>Practice within your course.</h2><p style={{ marginTop: 16, color: 'var(--ink)', fontSize: '0.875rem' }}>MET Mastery is included with your guided course, not a separate subscription. Talk to your teacher about which plan matches your test date.</p></div></section>

        <section id="signin" aria-labelledby="lp-signin-h" style={{ padding: '72px 0', background: 'var(--on-dark)', borderTop: '1px solid #E8E5DF' }}><div className="lp-wrap" style={{ maxWidth: 560, textAlign: 'center' }}><p className="lp-eyebrow">Sign in</p><h2 id="lp-signin-h" style={{ margin: 0, font: "600 clamp(1.85rem,4vw,3rem)/1.05 'Cormorant Garamond',serif" }}>Already a member?</h2><p style={{ marginTop: 12, color: 'var(--ink)', fontSize: '0.875rem' }}>Use the sign-in link your teacher shared, or return to your workspace.</p><p style={{ marginTop: 20 }}><button className="lp-btn lp-primary" type="button" onClick={onMemberSignIn}>Go to sign in <span aria-hidden="true">→</span></button></p></div></section>
      </main>
      <footer style={{ padding: '30px 0', color: 'var(--ink)', background: 'var(--on-dark)', borderTop: '1px solid #E8E5DF', fontSize: '0.75rem' }}><div className="lp-wrap" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20 }}><span style={{ font: "700 1.25rem 'Cormorant Garamond',serif" }}>MET Mastery</span><span>© 2026 MET Mastery</span></div></footer>
    </div>
  );
}
