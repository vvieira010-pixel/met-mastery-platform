...
const MODE_SUBTITLES = { grammar:'Tenses to inversion — 21 topics', vocab:'Work to media — 11 topics', reading:'51 passages · 102 questions', speaking:'Picture to persuasion — 14 topic paths', writing:'Short answer to essay — 138 tasks', listening:'MET-style drills — 63 clips' };
...
export default function PracticeStudio({ studentId, onBack: _onBack, "data-testid": testId }){
  ...
  useEffect(()=>{ if(!selectedKind) return; (async()=>{ if(selectedKind==='listening'){ setTopics(selectedListeningPart ? await getPracticeStudioListeningTopics(selectedListeningPart) : await getPracticeStudioListeningParts()); } else if(selectedKind==='speaking'){ setTopics([...getPracticeStudioSpeakingTopics(), ...getTopicList(selectedKind)]); } else setTopics(getTopicList(selectedKind)); })(); },[selectedKind,selectedListeningPart]);
  ...
  const showTopicPicker = selectedKind && (showListeningPartPicker || !selectedTopic);
  const selectedTopicTitle = topics.find(t=>t.id===selectedTopic)?.title||'';
  const showLanding = !selectedKind;
  const handleSelectMode=k=>{setSelectedKind(k); setSelectedTopic(null); setSelectedListeningPart(null); setSelectedListeningFormat('all'); setListeningSearch(''); setSessionKey(v=>v+1); setFadingVerdict(null); setSessionComplete(null);};
  const handleBackToLanding=()=>{setSelectedKind(null); setSelectedTopic(null); setSelectedListeningPart(null); setSelectedListeningFormat('all'); setListeningSearch(''); setExercises([]); setFadingVerdict(null); setSessionComplete(null);};
  const handleListeningPartSelect=partId=>{setSelectedListeningPart(partId); setSelectedTopic(null); setListeningSearch(''); setSessionComplete(null);};
      ...
      {showLanding ? (
        <>
          ...
          <section className="section">
            <div className="section-header">
              ...
              <span className="section-subtitle">218 Grammar · 128 Vocab · 102 Reading · 189 Speaking prompts · 138 Writing · 63 Listening clips</span>
            </div>
        ...
        <div className="section">
          <header className="student-page-header">
            <button onClick={showListeningPartPicker ? handleBackToLanding : ()=>selectedKind==='listening' ? setSelectedListeningPart(null) : handleBackToLanding()} className="student-page-back">← {showListeningPartPicker || selectedKind!=='listening' ? 'All skills' : 'Listening parts'}</button>
            <h1 className="student-page-title">{showListeningPartPicker ? 'MET-style Listening Practice' : MODE_LABELS[selectedKind]}</h1>
            <span className="student-page-subtitle">{showListeningPartPicker ? 'Choose a part-style drill, then choose a topic. These are extra practice, not a timed full MET form.' : selectedKind==='listening' ? 'Choose a topic within this listening part.' : MODE_SUBTITLES[selectedKind]}</span>
          </header>
          <div className="practice-studio-topics">
            ...
            <div className="topic-grid">
              {filteredTopics.map(t=>(
                <button key={t.id} onClick={()=>showListeningPartPicker ? handleListeningPartSelect(t.id) : setSelectedTopic(t.id)} className="topic-card">
        ...
        <div className="section practice-studio-exercise-shell" data-tour-target="practice-session">
          <header className="student-page-header">
            <button onClick={()=>setSelectedTopic(null)} className="student-page-back">← {selectedKind==='listening' ? 'Topics' : MODE_LABELS[selectedKind]}</button>
